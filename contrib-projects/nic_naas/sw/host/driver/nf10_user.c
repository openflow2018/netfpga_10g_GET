...
/* Base address for nf10_identifier_vx_xx_x pcore - Check XPS */
#define ID_BASE_ADDR			0x6a000000ULL
#define project_id(val)			(u16)(val & 0x3fffULL)
#define ref_or_contrib(val)		(u8)((val >> 15) & 1)
#define dma_id(val)				(u8)(val >> 16)
#define phy_init_status(val)	(u8)((val >> 24) & 1)
#define pipeline_reg(val)		(u8)((val >> 25) & 1)
#define test_suite(val)			(u8)((val >> 26) & 1)
#define tool_flow(val)			(u8)((val >> 27) & 0x1f)
#define tag_0(val)				(u8)(val & 0xf)
#define tag_1(val)				(u8)((val >> 4) & 0xf)
#define tag_2(val)				(u8)((val >> 8) & 0xf)
#define byte_ext(val, byte_pos)	(u8)((val >> 8 * byte_pos) & 0xff)

static void nf10_project_decoder(struct nf10_adapter *adapter);
...
int nf10_init_fops(struct nf10_adapter *adapter)
{
	...
	/* Read from the nf10_identifier_vx_xx_x pcore */
	nf10_project_decoder(adapter);
...
static void nf10_project_decoder(struct nf10_adapter *adapter)
{
	u64 addr, val, epoch_lower, epoch_upper;
	time_t tval;
	struct tm tm;

	/* Register 1: Holds the epoch lower 32 bits */
	addr = ID_BASE_ADDR;
	if (check_axi(read_axi(adapter, addr, &epoch_lower)))
		return;

	/* Register 2: Holds the epoch upper 32 bits */
	addr = ID_BASE_ADDR + 0x4;
	if (check_axi(read_axi(adapter, addr, &epoch_upper)))
		return;

	/* see linux/time.h */
	tval = (epoch_upper << 32) | epoch_lower;
	time_to_tm(tval, 0, &tm);
	pr_info("nf10: Bitfile implemented on %d:%d:%d on %d/%d/%ld\n", 
		tm.tm_hour, tm.tm_min, tm.tm_sec, tm.tm_mday, tm.tm_mon+1, tm.tm_year + 1900);

	/* Register 3: Holds project related information */
	addr = ID_BASE_ADDR + 0x8;
	if (check_axi(read_axi(adapter, addr, &val)))
		return;

	pr_info("nf10: System code: %x:%x:%x\n", dma_id(val), ref_or_contrib(val),
			project_id(val));

	/* Project name */
	switch(project_id(val)) {
		case 0x1:
			pr_info("nf10: reference_nic identified\n");
			break;
		case 0x2:
			pr_info("nf10: reference_switch identified\n");
			break;
		case 0x3:
			pr_info("nf10: reference_router identified\n");
			break;
		case 0x4:
			pr_info("nf10: reference_switch_lite identified\n");
			break;
		case 0x5:
			pr_info("nf10: nic_naas identified\n");
			break;
		default:
			pr_err("nf10: No project identified!\n");
			break;
	}

	/* Project source */
	switch(ref_or_contrib(val)) {
		case 0x0:
			pr_info("nf10: This is a contributed project\n");
			break;
		case 0x1:
			pr_info("nf10: This is a reference project\n");
			break;
		default:
			pr_err("nf10: Cannot identify the project!\n");
			break;
	}
	...
	switch(dma_id(val)) {
		case 0x1:
			pr_info("nf10: Project has DMA version 1\n");
			break;
		case 0x2:
			pr_info("nf10: Project has DMA version 2\n");
			break;
		case 0x3:
			pr_info("nf10: Project has DMA version 3\n");
			break;
		default:
			pr_err("nf10: DMA version is unidentified!\n");
			break;
	}

	/* PHY status */
	switch(phy_init_status(val)) {
		case 0x0:
			pr_info("nf10: Phy initialized by microblaze\n");
			break;
		case 0x1:
			pr_info("nf10: Phy requires initialization by software\n");
			break;
		default:
			pr_err("nf10: Cannot identify the project!\n");
			break;
	...
	/* Pipeline reg */
	switch(pipeline_reg(val)) {
		case 0x0:
			pr_info("nf10: This project doesn't have registers in the pipeline\n");
			break;
		case 0x1:
			pr_info("nf10: This project has registers in the pipeline\n");
			break;
		default:
			pr_err("nf10: Stautus of registers in pipeline is unknown!\n");
			break;
	}

	/* Test suite */
	switch(test_suite(val)) {
		case 0x0:
			pr_info("nf10: Test suite is not available for the project\n");
			break;
		case 0x1:
			pr_info("nf10: Test suite is available for the project\n");
			break;
		default:
			pr_err("nf10: Cannot determine the status of the test suite!\n");
			break;
	}

	/* Tool flow */
	switch(tool_flow(val)) {
		case 0x1:
			pr_info("nf10: The project uses Xilinx ISE tool flow\n");
			break;
		case 0x2:
			pr_info("nf10: The project uses Xilinx EDK tool flow\n");
			break;
		case 0x3:
			pr_info("nf10: The project uses Xilinx Vivado tool flow\n");
			break;
		default:
			pr_err("nf10: Cannot determine the tool flow for the project!\n");
			break;
	}

	/* Register 4: Holds git tag information */
	addr = ID_BASE_ADDR + 0xc;
	if (check_axi(read_axi(adapter, addr, &val)))
		return;

	pr_info("nf10: NetFPGA GitHub Tag: %d.%d.%d\n", tag_2(val), tag_1(val),
			tag_0(val));

	/* Register 5: Holds board specific information */
	addr = ID_BASE_ADDR + 0x10;
	if (check_axi(read_axi(adapter, addr, &val)))
		return;

	pr_info("nf10: Board Revision number   : %x\n", byte_ext(val,0));
	pr_info("nf10: Board idenfification id : %x\n", byte_ext(val,1));
	pr_info("nf10: Board PCB version       : %x\n", byte_ext(val,2));
	pr_info("nf10: Board Assembly number   : %x\n", byte_ext(val,3));

	/* Board name */
	switch(byte_ext(val,1)) {
		case 0x1:
			pr_info("nf10: This project is for NetFPGA-1G board\n");
			break;
		case 0x2:
			pr_info("nf10: This project is for NetFPGA-10G board\n");
			break;
		case 0x3:
			pr_info("nf10: This project is for NetFPGA-CML board\n");
			break;
		case 0x4:
			pr_info("nf10: This project is for NetFPGA-SUME board\n");
			break;
		default:
			pr_err("nf10: Board can't be identified!\n");
			break;
	}
}
